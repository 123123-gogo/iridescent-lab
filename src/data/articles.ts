...
const mira = getAuthor('mira')!
const kai = getAuthor('kai')!
...
以"IRIDESCENT"站点为例，我们的选择是：Noto Serif SC作为正文字体，16px字号，1.75行高，最大720px行长，纯白背景——每一个参数都经过推敲，只为让读者沉浸在内容中。
...
这也是为什么很多大品牌的蓝色看起来"差不多"——蓝色是满足这些条件最稳定的色系之一。IRIDESCENT选择的当前站点品牌色同理：清晰、稳定、跨设备。
...
在一个设计成"让你分心"的数字世界里，选择专注本身就是一种抵抗。IRIDESCENT这个项目，也是这种抵抗的一个小小实践。`,
